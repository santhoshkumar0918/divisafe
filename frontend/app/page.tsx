...
import { MessageCircle, Users, Shield, Heart, Brain, Lock, Menu, X, Camera, Upload, CheckCircle, AlertCircle, Loader2, LogIn, FileText, XCircle } from 'lucide-react'
import { motion, AnimatePresence } from 'framer-motion'
import { useRouter } from 'next/navigation'
        ...
        <header className="fixed top-0 w-full bg-black/50 backdrop-blur-xl border-b border-white/10 z-50">
          ...
          <AnimatePresence>
            {isMenuOpen && (
              <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="md:hidden overflow-hidden">
                <nav className="flex flex-col space-y-2 px-4 pt-2 pb-6 border-t border-white/10">
                  <a href="/" className="text-gray-300 hover:bg-white/10 rounded-lg px-4 py-3 transition-colors duration-200 font-medium">Home</a>
                  <a href="/support-rooms" className="text-gray-300 hover:bg-white/10 rounded-lg px-4 py-3 transition-colors duration-200 font-medium">Support Rooms</a>
                  <a href="/ai-support" className="text-gray-300 hover:bg-white/10 rounded-lg px-4 py-3 transition-colors duration-200 font-medium">AI Support</a>
                  <a href="/counselors" className="text-gray-300 hover:bg-white/10 rounded-lg px-4 py-3 transition-colors duration-200 font-medium">Counselors</a>
                  <button
        ...
        {isLoginModalOpen && (
          ...
          >
            ...
            >
              <div className="p-8">
                <div className="flex items-center justify-between mb-6">
                  <h2 className="text-2xl font-bold text-white">Secure Anonymous Login</h2>
                  <button onClick={() => setIsLoginModalOpen(false)} className="p-2 text-gray-400 hover:text-white transition-colors rounded-full hover:bg-white/10"><X className="w-6 h-6" /></button>
                </div>
                <MockQRCodePassportLogin onSuccess={handleLoginSuccess} onError={handleLoginError} />
              </div>
            </motion.div>
          </motion.div>
        )}
...
// Sub-components like NavLink, QuickAccessCard, etc. remain the same.

function NavLink({ href, children }: { href: string; children: React.ReactNode }) {
  return (
    <a href={href} className="text-gray-300 hover:text-white transition-colors duration-200 font-medium relative group">
      {children}
      <span className="absolute left-0 bottom-[-4px] w-0 h-0.5 bg-gradient-to-r from-teal-400 to-cyan-400 transition-all duration-300 group-hover:w-full"></span>
    </a>
  )
...
// ... QuickAccessCard, FeatureCard, FooterLinks ...

// --- ENHANCED AND FUNCTIONAL LOGIN MODAL COMPONENT ---
function MockQRCodePassportLogin({ onSuccess, onError }: { onSuccess: (p: any) => void, onError: (e: string) => void }) {
  const [selectedMethod, setSelectedMethod] = useState<'qr' | 'manual' | null>(null)
  const [verificationStatus, setVerificationStatus] = useState<'idle' | 'processing' | 'success' | 'error'>('idle')
  const [progress, setProgress] = useState(0)
  const [currentStep, setCurrentStep] = useState('')
  ...
  const handleStartVerification = () => {
    if (selectedMethod === 'qr' || (selectedMethod === 'manual' && uploadedFile)) {
      simulateVerification()
    }
  }
    ...
    return (
      <div className="text-center p-8">
        ...
        <div className="w-full bg-gray-700/50 rounded-full h-2.5">
          <div className="bg-gradient-to-r from-teal-500 to-cyan-500 h-2.5 rounded-full transition-all duration-500" style={{ width: `${progress}%` }}></div>
        </div>
      </div>
    )
  }
  ...
  if (!selectedMethod) {
    return (
      <div className="text-center">
        <p className="text-gray-400 mb-6">Your identity is verified on-device. No personal data is stored on our servers.</p>
        <div className="grid md:grid-cols-2 gap-6">
          <MethodButton icon={<Camera />} title="QR Code Scan" description="Quick verification via mobile." onClick={() => setSelectedMethod('qr')} />
          <MethodButton icon={<Upload />} title="Manual Upload" description="Upload your passport or ID." onClick={() => setSelectedMethod('manual')} />
        </div>
      </div>
    )
  }
  ...
  return (
    <div>
      <button onClick={() => { setUploadedFile(null); setSelectedMethod(null); }} className="text-sm text-gray-400 hover:text-white mb-4 flex items-center">
      ...
      {selectedMethod === 'qr' && (
        <div className="text-center p-8 bg-gray-800/50 rounded-xl border border-white/10">
          ...
          <h3 className="text-xl font-bold text-white mb-2">Scan with your Device</h3>
          <p className="text-gray-400 mb-6">A mock QR code would appear here for you to scan.</p>
          {/* Placeholder for QR code image */}
          <div className="w-48 h-48 bg-white mx-auto rounded-lg flex items-center justify-center text-black">Mock QR Code</div>
        </div>
      )}
        ...
        <div>
          {!uploadedFile ? (
            <div
              ...
              className={`p-8 text-center bg-gray-800/50 rounded-xl border-2 border-dashed transition-colors ${isDragOver ? 'border-cyan-400 bg-gray-700/50' : 'border-white/10'} cursor-pointer`}
            >
              <Upload className="w-12 h-12 text-gray-400 mx-auto mb-4" />
              <h3 className="text-xl font-bold text-white mb-2">Drag & Drop Your Document</h3>
            ...
            <div className="p-6 text-center bg-gray-800/50 rounded-xl border border-white/10 flex items-center justify-between">
              ...
              <button onClick={() => setUploadedFile(null)} className="p-2 text-gray-400 hover:text-white transition-colors rounded-full hover:bg-white/10"><XCircle className="w-6 h-6" /></button>
            </div>
          )}
        </div>
      )}
      ...
      <div className="mt-6">
        <button
          onClick={handleStartVerification}
          disabled={verificationStatus !== 'idle' || (selectedMethod === 'manual' && !uploadedFile)}
...
function MethodButton({ icon, title, description, onClick }: { icon: React.ReactNode, title: string, description: string, onClick: () => void }) {
  return (
    <button onClick={onClick} className="p-6 bg-gray-800/50 rounded-xl border border-white/10 text-center hover:border-cyan-400/50 hover:bg-gray-700/50 transition-all group">
